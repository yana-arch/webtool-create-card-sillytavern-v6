...
export const CodeDisplay: React.FC<CodeDisplayProps> = ({ jsonString, fileName = 'character_card' }) => {
    ...
    <div className="bg-gray-800/50 rounded-xl border border-gray-700 h-full flex flex-col">
      <div className="flex justify-between items-center p-3 bg-gray-900/40 rounded-t-xl border-b border-gray-700">
        ...
        <div className="flex gap-2">
            <button onClick={handleCopy} className="text-xs bg-purple-600/50 hover:bg-purple-500/50 text-white py-1 px-3 rounded-md transition-colors">
                {copyText}
            </button>
            <button onClick={handleDownload} className="text-xs bg-pink-600/50 hover:bg-pink-500/50 text-white py-1 px-3 rounded-md transition-colors">
                Tải xuống JSON
            </button>
        </div>
      </div>
