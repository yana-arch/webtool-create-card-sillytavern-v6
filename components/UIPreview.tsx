...
export const UIPreview: React.FC<UIPreviewProps> = ({ name, htmlCode, isSelected, onSelect, onEdit }) => {
  ...
  const fullHtml = `
    ...
    <html>
      ...
      <body style="margin: 0; padding: 0; background-color: #1a202c; overflow-x: hidden;">
        ${htmlCode.replace(/<DATA_BLOCK>[\s\S]*?<\/DATA_BLOCK>/s, `
  ...
  return (
    <div className={`relative border-2 rounded-lg transition-all duration-300 ${isSelected ? 'border-purple-500 shadow-2xl scale-105' : 'border-gray-700 hover:border-purple-700'}`}>
      <div className="p-3 bg-gray-900/60 rounded-t-lg flex justify-between items-center">
        ...
        <button onClick={() => setIsExpanded(!isExpanded)} className="text-xs bg-gray-600 hover:bg-gray-700 text-white py-1 px-2 rounded-md transition-colors">
