...
async function callGeminiWithRetry(
  ...
  for (const key of orderedKeys) {
    try {
      ...
      if (
        ...
        console.warn(
          `API key ...${key.slice(-4)} thất bại. Đang thử key tiếp theo. Lỗi: ${e.message}`,
        );
  ...
  throw new Error(
    `Tất cả các API key đều thất bại. Lỗi cuối cùng: ${lastError?.message || "Lỗi không xác định"}`,
  );
}

const buildKnowledgePromptSection = (
  library: KnowledgeFile[],
  referenceId?: string,
  context: "create" | "fix" = "create",
): string => {
  if (library.length === 0) return "";
  let knowledgePrompt = `\n**THƯ VIỆN KIẾN THỨC (Nguồn tham khảo):**\n`;
  const referenceFile = referenceId
    ? library.find((f) => f.id === referenceId)
    : null;
            ...
            uiPreviews: {
              ...
              items: {
                ...
                properties: {
                  name: {
                    type: Type.STRING,
                    description: "Tên sáng tạo cho mẫu giao diện.",
                  },
                  code: {
                    type: Type.STRING,
                    description:
                      "Mã HTML, CSS, và JS hoàn chỉnh cho giao diện.",
                  },
                },
                required: ["name", "code"],
              },
...
export const generateWelcomeScreenPreviews = async (
  apiKey: string,
  theme: string,
  options: {
    tone: string;
...
export const generateCreatorPreviews = async (
  apiKey: string,
  theme: string,
  description: string,
  count: number,
...
export const generateStatusPanelPreviews = async (
  apiKey: string,
  theme: string,
  description: string,
  count: number,
...
export const generateRegexUIPreviews = async (
  apiKey: string,
  description: string,
  theme: string,
  count: number,
...
export const editUIPreview = async (
  apiKey: string,
  currentCode: string,
  editRequest: string,
): Promise<string> => {
...
**QUAN TRỌNG:** Chỉ trả về đoạn mã HTML đã được sửa đổi. KHÔNG thêm bất kỳ giải thích nào. Toàn bộ đầu ra của bạn phải nằm trong một đối tượng JSON duy nhất có khóa là "updatedCode".`;
    ...
    config: {
      ...
      responseSchema: {
        ...
        properties: {
          updatedCode: {
            type: Type.STRING,
            description: "Mã HTML, CSS và JS đã được sửa đổi.",
          },
        },
  ...
  try {
    ...
    throw new Error(
      "AI đã trả về một định dạng JSON không hợp lệ khi chỉnh sửa.",
    );
  }
};
            ...
            entries: {
              ...
              items: {
                ...
                properties: {
                  ...
                  insertion_order: { type: Type.INTEGER },
                },
              },
            },
          },
...
export const analyzeLorebookForSuggestions = async (
  apiKey: string,
  lorebookContent: string,
): Promise<{ name: string; theme: string; firstMessageIdea: string }> => {
...
**QUAN TRỌNG:** Cung cấp đầu ra dưới dạng một đối tượng JSON duy nhất theo schema.`;
    ...
    config: {
      ...
      responseSchema: {
        ...
        properties: {
          name: {
            type: Type.STRING,
            description: "Tên nhân vật/thẻ được đề xuất.",
          },
          theme: {
            type: Type.STRING,
            description:
              "Các chủ đề và từ khóa được đề xuất, cách nhau bởi dấu phẩy.",
          },
          firstMessageIdea: {
            type: Type.STRING,
            description: "Ý tưởng cho tin nhắn đầu tiên, phù hợp với lore.",
          },
        },
  ...
  try {
    ...
    console.error(
      "Lỗi phân tích JSON từ analyzeLorebookForSuggestions:",
      response.text,
      e,
    );
    throw new Error(
      "AI đã trả về một định dạng JSON không hợp lệ cho các đề xuất.",
    );
  }
};
...
export const generateSillyTavernCard = async (
  ...
  const knowledgePrompt = buildKnowledgePromptSection(
    knowledgeLibrary,
    options.referenceCardId,
    "create",
  );
...
`;
    ...
    const customLorePrompt =
      options.customLoreRequests &&
      options.customLoreRequests.filter((req) => req.trim() !== "").length > 0
        ? `
...
Tạo thẻ ngay bây giờ.`;
  ...
  try {
    ...
    if (!parsedJson.data.extensions.regex_scripts)
      parsedJson.data.extensions.regex_scripts = [];
    if (!parsedJson.data.extensions.TavernHelper_scripts)
      parsedJson.data.extensions.TavernHelper_scripts = [];
    if (!parsedJson.data.alternate_greetings)
      parsedJson.data.alternate_greetings = [];

    const createRegexScript = (
      id: string,
      name: string,
      trigger: string,
      code: string,
      ...
      runOnEdit: true,
      maxDepth:
        trigger.includes("Bắt đầu") || trigger.includes("Mở đầu") ? 1 : null,
    });

    // 1. Status Panel
    if (options.statusPanelCode) {
      parsedJson.data.extensions.regex_scripts.push(
        createRegexScript(
          "STATUS_PANEL_V1",
          "[UI] Bảng Trạng thái",
          "/<DATA_BLOCK>([\\s\\S]*?)<\\/DATA_BLOCK>/s",
          options.statusPanelCode,
        ),
      );
    }
    ...
    if (options.welcomeScreenCode) {
      parsedJson.data.extensions.regex_scripts.push(
        createRegexScript(
          "WELCOME_SCREEN_V1",
          "[UI] Màn hình Chào mừng",
          welcomeTrigger,
          options.welcomeScreenCode,
        ),
      );
    }
    ...
    if (features.includes("progressionSystem")) {
      if (
        !parsedJson.data.extensions.TavernHelper_scripts.some(
          (s: any) => s.value.id === MVU_HELPER_SCRIPT.value.id,
        )
      ) {
    ...
    throw new Error(
      `AI đã trả về một định dạng JSON không hợp lệ. Vui lòng thử lại.`,
    );
  }
};

export const addLoreEntries = async (
  apiKey: string,
...
**Yêu cầu:** Tạo ${count} mục lore mới (nhân vật phụ, địa điểm, lịch sử, v.v.). Mỗi mục phải có 'keys', 'content', và 'comment'. Chỉ trả về một mảng JSON chứa các mục mới trong khóa 'new_entries'.`;
  ...
  try {
    ...
    if (!updatedCard.data.character_book)
      updatedCard.data.character_book = {
        name: `${card.name} Lorebook`,
        entries: [],
      };
    let maxOrder = updatedCard.data.character_book.entries.reduce(
      (max, e) => Math.max(max, e.insertion_order || 0),
      0,
    );
    newEntries.forEach((entry: any) => {
      updatedCard.data.character_book.entries.push({
        ...entry,
        enabled: true,
        insertion_order: ++maxOrder,
      });
    });
    return updatedCard;
  } catch (e) {
...
export const generateAndAddRegex = async (
  apiKey: string,
  card: SillyTavernCard,
  description: string,
  uiCode: string,
...
3.  **Kết hợp:** Trả về một JSON duy nhất chứa 'regex_script' (scriptName, findRegex) và 'lorebook_entry' (keys, content, comment).`;
  ...
  try {
    ...
    if (!updatedCard.data.extensions.regex_scripts)
      updatedCard.data.extensions.regex_scripts = [];
    if (!updatedCard.data.character_book)
      updatedCard.data.character_book = {
        name: `${card.name} Lorebook`,
        entries: [],
    ...
    const newRegexScript = {
      id: `CUSTOM_REGEX_${Date.now()}`,
      scriptName: `[CUSTOM] ${parsed.regex_script.scriptName}`,
      findRegex: parsed.regex_script.findRegex,
      replaceString: `\`\`\`html\n${uiCode}\n\`\`\``,
      placement: [2],
      disabled: false,
      promptOnly: false,
    ...
    const maxOrder = updatedCard.data.character_book.entries.reduce(
      (max, e) => Math.max(max, e.insertion_order || 0),
      0,
    );
    const newLoreEntry = {
      ...parsed.lorebook_entry,
      enabled: true,
...
Sửa thẻ ngay bây giờ.`;
  ...
  try {
    ...
    console.error(
      "Lỗi phân tích JSON từ fixSillyTavernCard:",
      response.text,
      e,
    );
    throw new Error(
      "AI đã trả về một định dạng JSON không hợp lệ sau khi sửa lỗi.",
    );
  }
};
